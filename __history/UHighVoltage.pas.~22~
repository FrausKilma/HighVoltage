...
uses
  ...
  Vcl.Imaging.jpeg, IdHTTP, UArdEmul, cxGraphics, cxControls, cxLookAndFeels,
  cxLookAndFeelPainters, cxContainer, cxEdit, cxTextEdit, cxMaskEdit,
  cxDropDownEdit, cxImageComboBox, IdGlobal, UMainThread;
...
type
  TMainForm = class(TForm)
    ...
    procedure btnSendPanelClick(Sender: TObject);
  private
...
procedure TMainForm.btnSendPanelClick(Sender: TObject);
begin
if cxImageComboBox1.ItemIndex > -1 then
 begin
  case cxImageComboBox1.EditingValue of
   0: SendPanel('Init');
   1: SendPanel('Step1');
   2: SendPanel('Step2');
   3: SendPanel('Step3');
...
//     ShowMessage('Host: ' + ClientPanel.Host + ' Port: ' +IntToStr(ClientPanel.Port));
  ...
  //        FrmDebug.MemSend.Lines.Add('(' + txt +') ' + SL[i]);
       ...
       memoPanel.Lines.add('Send complected. Count file send : ' + IntToStr(SL.Count));
...
//  Showmessage(inttostr(IdTCPServer1.DefaultPort));
  ...
  if Pos('LED1',strText) > 0 then SendPanel('Step1');
  if Pos('LED2',strText) > 0 then SendPanel('Step2');
  if Pos('LED3',strText) > 0 then SendPanel('Step3');
  if Pos('LED4',strText) > 0 then SendPanel('Step4');
  if Pos('LED5',strText) > 0 then SendPanel('Step5');
  if Pos('LED6',strText) > 0 then SendPanel('Step6');
  if Pos('LED7',strText) > 0 then SendPanel('Step7');
  if Pos('LEDINIT',strText) > 0 then SendPanel('Init');
  ...
  for i := Low(StrText) to High(StrText) do
   ...
   if i = Low(StrText) then
    if (StrText[i] = 'T') or (StrText[i] = 'S') then
     begin
      startedQuest := true;
...
//     if i = Length(Sensors)+Length(Relays) + 2 then
     if i = 25  then
      if i > 2 then
       if (strText[i-1] = 'T') or (strText[i-1] = 'F') then
        if (strText[i-2] = 'T') or (strText[i-2] = 'F') then
        if StrText[i] = 'T' then
         begin
          questComplit := true;
          startedQuest := false;
