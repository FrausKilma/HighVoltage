...
var
  ...
  Relays:array[0..7] of Integer;
  Sensors: array[0..11] of Integer;
  StepSend: array[0..11] of Integer;
  timeQuest : integer;
...
begin
 StepSend[i] := 1;
 case i of
   // сработал первый степ открываются релюшки во вторую дверь
   0: begin
   ...
   // срабатывание второго степа дальномер на клетке, релюшка на жаровне
   1: begin

       IRelay2.Brush.Color := clGreen;
       Relays[1] := 1;
       SendPanel('Step1');

      end;
   // срабатыванеи 3го степа дальномеры в гробу, релюшка на символы 1,2,3,4
   2: begin
       IRelay3.Brush.Color := clGreen;
       Relays[2] := 1;
       SendPanel('Step2');
      end;
   // срабатывание 4го степа геркон на крышке гроба, релюшка на символы 5,6,7,8
   3: begin
       IRelay4.Brush.Color:= clGreen;
       Relays[3] := 1;
        SendPanel('Step3');
      end;
   // срабатывание 5го степа: ёмкостной датчик 1 и релюшка на маске
   4: begin
   ...
   // Срабатывание 6го степа: емкостной датчик 2 и релюшка на выход из 2ой комнаты в 3ую
   5: begin
   ...
   // Срабатывание 7го степа: геркон на факелах и релюшка на выход из квеста
   6: begin
...
//  Showmessage(inttostr(IdTCPServer1.DefaultPort));
  ...
  if strText = 'LED1' then SendPanel('Step1');
  if strText = 'LED2' then SendPanel('Step2');
  if strText = 'LED3' then SendPanel('Step3');
  if strText = 'LED4' then SendPanel('Step4');
  if strText = 'LED5' then SendPanel('Step5');
  if strText = 'LED6' then SendPanel('Step6');
  if strText = 'LED7' then SendPanel('Step7');
  if strText = 'LEDINIT' then SendPanel('Init');
